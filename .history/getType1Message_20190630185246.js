...
var type1Message = {
    ...
    DomainNameFields: {
        DomainNameLen : 0, //2 bytes
        DomainNameMaxLen: 0, // 2 bytes
        DomainNameBufferOffset:  2// 4 bytes, from neginning of message to deomain name in payload 
    },  // 8bytes if NTLMSSP_NEGOTIATE_OEM_DOMAIN_SUPPLIED  is set
    WorkstationFields: {
        WorkstationLen : 0, // 2 bytes 
        WorkstationMaxLen: 0, // 2 bytes 
        WorkstationBufferOffset: 0// 4 bytes 
    }, // 8 bytes NTLMSSP_NEGOTIATE_OEM_WORKSTATION_SUPPLIED 
    Version: 2,// 8 bytes 
    NegotiateFlags:  0,// for debugging purpose 
    Payload: 0// 1 byte array 
};

// var bytes = []

// the Negotiate flags are constructed by lieele endian, 0x0582000 should be read as 82 05 
// a good answer can be found here: https://stackoverflow.com/questions/46450826/understanding-the-negotiateflags-parameter-of-ntlm-messages
// This article is awesome at explaining NTLM technical implementation: http://davenport.sourceforge.net/ntlm.html#theNtlmFlags
...
negotiateFlags = {
    ...
    // D requests session key negotiation for message signature
    NTLMSSP_NEGOTIATE_SIGN: 1,
    r10: 0,
    // C target name field in the challenga_message
    NTLMSSP_REQUEST_TARGET: 0,
    // B OEM character set encoding
    NTLM_NEGOTIATE_OEM: 0,
    // A Unicode character set encoding
    NTLMSSP_NEGOTIATE_UNICODE: 0
};

exports.getType1Message = function() {
    // construct the type 1 message here in byte array 
}